...
class Speaker():
            ...
            if ended[i]:                                            # Just ignore this index
                ...
                for k, candidate in enumerate(ob['candidate']):
                    if candidate['viewpointId'] == ob['teacher']:   # Next view point
                        a[i] = k
                        next_action[i] = [candidate['pointId'], candidate['heading'], candidate['elevation']]
                        break
                else:   # Stop here
                    assert ob['teacher'] == ob['viewpoint']         # The teacher action should be "STAY HERE"
                    a[i] = len(ob['candidate'])
        return torch.from_numpy(a).cuda(), next_action
    ...
    def from_shortest_path(self, viewpoints=None, get_first_feat=False):
        ...
        while not ended.all():
            ...
            for ob in obs:
                tmp_path_views.append(ob["viewpoint"])
                tmp_path_view_index.append(ob["viewIndex"])
                tmp_path_heading.append(ob['heading'])
                tmp_path_elevation.append(ob['elevation'])
                
            path_views.append(tmp_path_views)
            path_view_index.append(tmp_path_view_index)
            path_heading.append(tmp_path_heading)
            path_elevation.append(tmp_path_elevation)

            img_feats.append(self.listener._feature_variable(obs))
            teacher_action, potential_actions = self._teacher_action(obs, ended)
            teacher_action = teacher_action.cpu().numpy()
            teacher_action_list.append(potential_actions)
        ...
        new_path_elevation = []
        action_list = []
        for id, each_len in enumerate(length):
            new_path_views.append(list(list(zip(*path_views))[id][:each_len]))
            new_path_view_index.append(list(list(zip(*path_view_index))[id][:each_len]))
            new_path_heading.append(list(list(zip(*path_heading))[id][:each_len]))
            new_path_elevation.append(list(list(zip(*path_elevation))[id][:each_len]))
            action_list.append(list(list(zip(*teacher_action_list))[id][:each_len]))
        
        for ob_id, ob in enumerate(obs):
            tmp_dict = {}
            tmp_dict['instr_id'] = ob['instr_id']
            tmp_all = [[ob['scan']]*length[ob_id], new_path_views[ob_id], new_path_view_index[ob_id], \
                        new_path_heading[ob_id], new_path_elevation[ob_id]]
            tmp_dict['path']= list(zip(*tmp_all))
            tmp_dict['teacher_action_emd'] = action_list[ob_id]
            tmp_dict['instr_encoding'] = ob['instr_encoding']
            if tmp_dict not in self.aug_dict:
                self.aug_dict.append(tmp_dict)
