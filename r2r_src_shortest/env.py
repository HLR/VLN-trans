...
from utils import load_datasets, load_nav_graphs, pad_instr_tokens, print_progress
...
class R2RBatch():
    ...
    def __init__(self, feature_store, nav_graphs, batch_size=100, seed=10, splits=['train'], tokenizer=None,
                 name=None): 
        ...
        scans = []
        for split in splits:
            input_data = load_datasets([split])
            for i_item, item in enumerate(input_data):
                ...
                if "/" in split:
                    ...
                    # Change this part for different data structure
                    '''RXR
                    new_item = dict(item)
                    ...
                    if new_item['instr_encoding'] is not None:  # Filter the wrong data
                        self.data.append(new_item)
                        scans.append(item['scan'])
                    print_progress(i_item, len(input_data), prefix='Progress:', suffix='Complete', bar_length=50)
    ...
    def _next_path_pano_action(self, state, adj_loc_list, path, goalViewpointId):
        if state.location.viewpointId == goalViewpointId:
            return 0  # do nothing
        next_index = path.index(state.location.viewpointId) + 1
        nextViewpointId = path[next_index]
        for n_a, loc_attr in enumerate(adj_loc_list):
            if loc_attr['nextViewpointId'] == nextViewpointId:
                return n_a


    def _get_panorama_states(self, state, nav_graphs):
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            ground_path = item['path']
            adj_loc_list = self._get_panorama_states(state, self.nav_graphs)
            # should change this one to _next_path_pano_action if full path is provided. 
            # but _shortest_path_pano_action if only start and end viewpoints are provided
    ...
    def _adjust_heading(self, sim, heading):
    ...
    def _navigate_to_location(self, sim, nextViewpointId, absViewIndex):
        ...
        self._adjust_heading(sim, absViewIndex % 12 - state.viewIndex % 12)
        self._adjust_elevation(sim, absViewIndex // 12 - state.viewIndex // 12)
    ...
    def step(self, actions, last_obs=None, world_states=None):
        ...
        if world_states is None:
            for i, (sim, action, last_ob) in enumerate(zip(self.env.sims, actions, last_obs)):
                ...
                self._navigate_to_location(sim, loc_attr['nextViewpointId'], loc_attr['absViewIndex'])
