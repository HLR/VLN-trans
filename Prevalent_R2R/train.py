...
from model import EncoderLSTM, EncoderMultiLSTM, BertEncoder, MultiBertEncoder, GptEncoder, MultiGptEncoder,\
    TransformerEncoder, MultiTransformerEncoder, BertImgEncoder, BertAddEncoder,MultiVilBertEncoder, MultiVilAddEncoder, \
    MultiAddLoadEncoder, AttnDecoderLSTM,HugLangEncoder,HugAddEncoder,MultiHugAddEncoder,VicEncoder,MultiVicEncoder,DicEncoder,MultiDicEncoder
from pytorch_transformers import BertForMaskedLM,BertTokenizer
from agent import Seq2SeqAgent
from eval import Evaluation
from feature import Feature
from pretrain_class import HugAddActionPreTrain,VicAddActionPreTrain,DicAddActionPreTrain
import pprint
import pdb
...
if __name__ == "__main__":
    ...
    parser.add_argument('--update_add_layer', dest='update_add_layer', type=boolean_string, default=True, help='update additional bert layer')
...
update_add_layer = params['update_add_layer']
...
use_bert = (encoder_type in ['bert', 'gpt','vlbert','MultiDicEncoder','MultiDicEncoder'])  # for tokenizer and dataloader
...
def train_val(n_iters_resume=0):
    ...
    # Build models and train
    #enc_hidden_size = hidden_size // 2 if bidirectional else hidden_size
    if encoder_type in ['vlbert','MultiVicEncoder', 'MultiDicEncoder']:
        if att_ctx_merge in ['mean','cat','max','sum']:
            if pretrain_model_name is not None:
                ...
                if encoder_type == 'MultiVicEncoder':
                    encoder = MultiVicEncoder(FEATURE_ALL_SIZE, enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers,reverse_input, top_lstm, multi_share, pretrain_n_sentences, vl_layers, bert_type)
                    premodel = VicAddActionPreTrain.from_pretrained(pretrain_model_name)
                elif encoder_type == 'MultiDicEncoder':
                    encoder = MultiDicEncoder(FEATURE_ALL_SIZE, enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers,reverse_input, top_lstm, multi_share, pretrain_n_sentences, vl_layers, la_layers,bert_type)
                    premodel = DicAddActionPreTrain.from_pretrained(pretrain_model_name)
                else:
                    encoder = MultiHugAddEncoder(FEATURE_ALL_SIZE, enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers,reverse_input, top_lstm, multi_share, pretrain_n_sentences, vl_layers, bert_type)
                    premodel =HugAddActionPreTrain.from_pretrained(pretrain_model_name)

                encoder.drop = nn.Dropout(p=args.dropout_ratio)
                encoder.bert = premodel.bert
                encoder.bert.update_lang_bert, encoder.bert.config.update_lang_bert = transformer_update, transformer_update
                encoder.bert.update_add_layer, encoder.bert.config.update_add_layer = update_add_layer, update_add_layer
            else:
                ...
                if encoder_type == 'MultiVicEncoder':
                    ...
                    encoder = MultiDicEncoder(FEATURE_ALL_SIZE, enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers,reverse_input, top_lstm, multi_share, pretrain_n_sentences, vl_layers, la_layers, False, bert_type)
                else:
                    encoder = MultiHugAddEncoder(FEATURE_ALL_SIZE, enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers,reverse_input, top_lstm, multi_share, pretrain_n_sentences, vl_layers, bert_type)
            ...
            if pretrain_model_name is not None:
                ...
                """
                encoder = HugAddEncoder(FEATURE_ALL_SIZE,enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers, reverse_input, top_lstm,vl_layers,bert_type)
                encoder.pretrain = False
                premodel =HugAddActionPreTrain.from_pretrained(pretrain_model_name)
                encoder.bert = premodel.bert
                encoder.bert.update_lang_bert = transformer_update
                encoder.bert.update_add_layer = update_add_layer
                """
                encoder = VicEncoder(FEATURE_ALL_SIZE,enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers, reverse_input, top_lstm,vl_layers,bert_type)
                encoder.pretrain = False
                premodel = VicAddActionPreTrain.from_pretrained(pretrain_model_name)
                encoder.bert = premodel.bert
                encoder.bert.update_lang_bert = transformer_update
                ...
                #encoder = VicEncoder(FEATURE_ALL_SIZE,enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers, reverse_input, top_lstm,vl_layers,bert_type)
                encoder = DicEncoder(FEATURE_ALL_SIZE,enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers, reverse_input, top_lstm,vl_layers,bert_type)
                encoder.pretrain = False
        ...
        if att_ctx_merge in ['mean','cat','max','sum']:
            ...
            if pretrain_model_name is not None:
                ...
                encoder.bert = premodel.bert
                encoder.bert.update = transformer_update
    ...
    finetunes = None
    if finetune_iters > 0:
        finetune_env = R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=finetune_batchsize,
                          splits=train_splits, tokenizer=tok, att_ctx_merge=att_ctx_merge)
        finetune_valenvs = {split: (R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=finetune_batchsize,
             splits=[split], tokenizer=tok, att_ctx_merge=att_ctx_merge), Evaluation([split], encoder_type)) for split in val_splits}  # subgoal
        finetunes = (finetune_env, finetune_valenvs)
    ...
    print("Your final best model iter is %d" % (best_model_iter))
    return best_model_iter
...
def load_test(n_iters_resume):
    ...
    if hasattr(encoder, 'n_sentences') and single_sentence_test:
        ...
        val_envs = {split: (
        R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, beam_size=beam_size, batch_size=batch_size,
                 splits=[split], tokenizer=tok, att_ctx_merge='Eval'), Evaluation([split], encoder_type)) for split in val_splits}  # , subgoal
        loss_str, data_log = '', defaultdict(list)
