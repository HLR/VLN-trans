...
class Speaker():
    ...
    def __init__(self, env, listener, tok):
        ...
        self.decoder = model.SpeakerDecoder(self.tok.vocab_size, args.wemb, self.tok.vocab['[PAD]'],
        ...
        # Evaluation
        self.softmax_loss = torch.nn.CrossEntropyLoss(ignore_index=self.tok.vocab['[PAD]'])
        self.triplet_loss = torch.nn.TripletMarginLoss()
        self.softmax_speaker = torch.nn.Softmax(dim=-1)

        # Will be used in beam search
        self.nonreduced_softmax_loss = torch.nn.CrossEntropyLoss(
            ignore_index=self.tok.vocab['[PAD]'],
            size_average=False,
            reduce=False
        )
    ...
    def train(self, iters):
        for i in range(iters):
            ...
            loss,_ = self.teacher_forcing(train=True)
    ...
    def gt_words(self, obs):
        ...
        pos_tensor = np.array([ob['pos_instr_encoding'] for ob in obs])
        neg_tensor = np.array([ob['neg_instr_encoding'] for ob in obs])
        mask_tensor = np.array([ob['sample_mask'] for ob in obs])

        # target_tesor = np.array([ob['target_encoding'] for ob in obs])
        return torch.from_numpy(seq_tensor).cuda(), torch.from_numpy(pos_tensor).cuda(), \
               torch.from_numpy(neg_tensor).cuda(), torch.from_numpy(mask_tensor).cuda()

    def teacher_forcing(self, train=True, features=None, insts=None, for_listener=False, target=None):
        if train:
            self.encoder.train()
            self.decoder.train()
        else:
        ...
        # Get Language Input
        if insts is None and target is None:
            insts, pos, neg, sample_mask = self.gt_words(obs)                                       # Language Feature

        # Decode
        logits, _, _, out_repr = self.decoder(insts, ctx, ctx_mask, h_t, c_t)
        _, _, _, out_repr_pos = self.decoder(pos, ctx, ctx_mask, h_t, c_t)
        neg_size= neg.shape[1]
        batch_size, cand_size, dim_size = ctx.shape
        neg_num = 3
        neg = neg.view(-1, 80)
        neg_ctx = ctx.unsqueeze(1).expand(batch_size, neg_size, cand_size, dim_size).reshape(-1, cand_size, dim_size)
        neg_ctx_mask = ctx_mask.unsqueeze(1).expand(batch_size, neg_size, cand_size).reshape(-1, cand_size)
        neg_h_t = h_t.unsqueeze(2).expand(1, batch_size, neg_size, args.rnn_dim).reshape(1, -1, args.rnn_dim)
        neg_c_t = c_t.unsqueeze(2).expand(1, batch_size, neg_size, args.rnn_dim).reshape(1, -1, args.rnn_dim)
        _, _, _, out_repr_neg = self.decoder(neg, neg_ctx, neg_ctx_mask, neg_h_t, neg_c_t)
        ...
        split_logits = logits.permute(0, 2, 1).contiguous()
        sub_loss = self.softmax_loss(
            input  = split_logits[:, :, :-1],         # -1 for aligning
            target = insts[:, 1:]               # "1:" to ignore the word <BOS>
        )
        anchor = out_repr
        anchor_pos = out_repr_pos
        anchor_neg = out_repr_neg.view(batch_size, 3, -1, dim_size)
        trip_loss = 0
        for i in range(neg_num):
            trip_loss += self.triplet_loss(anchor, anchor_pos, anchor_neg[:,i,:])
        
        trip_loss = trip_loss/neg_num
        ...
        loss =  sub_loss + trip_loss
        if train:
            return loss, out_repr
        else:
            # Evaluation
            _, predict = split_logits.max(dim=1)                                  # BATCH, LENGTH
    ...
    def infer_batch(self, sampling=False, train=False, featdropmask=None):
        ...
        word = np.ones(len(obs), np.int64) * self.tok.vocab['[CLS]']    # First word is <BOS>
        word = torch.from_numpy(word).view(-1, 1).cuda()
        for i in range(args.maxDecode):
            # Decode Step
            logits, h_t, c_t, _ = self.decoder(word, ctx, ctx_mask, h_t, c_t)      # Decode, logits: (b, 1, vocab_size)
