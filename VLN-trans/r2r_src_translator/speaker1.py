...
class Speaker():
    ...
    def __init__(self, env, listener, tok):
        ...
        self.triplet_loss = torch.nn.TripletMarginLoss(margin=10.0, p=2)
    ...
    def teacher_forcing(self, train=True, features=None, insts=None, for_listener=False, target=None):
        ...
        anchor =  self.softmax_speaker(logits).max(-1)[1][:, :10]
        anchor_pos = pos[:,:10]
        anchor_neg = neg[:,:,:10]
        trip_loss = 0
        ...
        trip_loss = trip_loss/neg.shape[1]
