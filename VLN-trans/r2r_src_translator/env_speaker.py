...
from utils import load_datasets, load_nav_graphs, pad_instr_tokens, length2mask
import torch
        ...
        for split in splits:
            for i_item, item in enumerate(tqdm(load_datasets([split]))):
                ...
                if "/" in split:
                    ...
                    max_len = args.maxInput
                    for key, value in item['samples'].items():
                        pos_instr_encoding = np.zeros(max_len, dtype=np.int64)
                        neg_instr_encoding = np.zeros((3, max_len), dtype=np.int64)
                        pos_instr_encoding[:len(value['pos_instr_encoding'])] = np.array(value['pos_instr_encoding'])
                        value['pos_instr_encoding'] = pos_instr_encoding
                        for neg_id, neg in enumerate(value['neg_instr_encoding']):
                            neg_instr_encoding[neg_id][:len(neg)] = np.array(neg)
                        value['neg_instr_encoding'] = neg_instr_encoding
                        value['mask'] = 1*(np.concatenate([np.expand_dims(pos_instr_encoding, axis=0), neg_instr_encoding], axis=0)>0)
                        assert value['mask'].shape[0] == 4
                    ...
                    for j, instr in enumerate(item['instructions']):
                        ...
                        for key, value in item['samples'].items():
                            ...
                            pos_instr_encoding = tokenizer.convert_tokens_to_ids(padded_pos_tokens)
                            new_item['samples'][key]['pos_instr_encoding'] = pos_instr_encoding
                            
                            ### neg_instr_encoding
                            neg_instr = value['neg_soft_instr'] + value['neg_hard_instr']
                            neg_instr_encoding = np.zeros((3, args.maxInput), dtype=np.int64)
                            ...
                            for n1 in range(0, 3):
                                ...
                                neg_instr_encoding[n1] = tokenizer.convert_tokens_to_ids(padded_neg_tokens)
                              
                            new_item['samples'][key]['neg_instr_encoding'] = neg_instr_encoding
                            new_item['samples'][key]['mask'] = 1*(np.concatenate([np.expand_dims(pos_instr_encoding, axis=0), neg_instr_encoding], axis=0)>0)
                            assert new_item['samples'][key]['mask'].shape[0] == 4
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if 'instr_encoding' in item:
                obs[-1]['instr_encoding'] = item['samples'][state.location.viewpointId]['pos_instr_encoding']
                obs[-1]['pos_instr_encoding'] = item['samples'][state.location.viewpointId]['pos_instr_encoding']
                obs[-1]['neg_instr_encoding'] = item['samples'][state.location.viewpointId]['neg_instr_encoding']
                obs[-1]['sample_mask'] = item['samples'][state.location.viewpointId]['mask']
