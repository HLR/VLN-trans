...
class SpeakerEncoder(nn.Module):
    ...
    def forward(self, action_embeds, feature, lengths, already_dropfeat=False):
        ...
        feature = feature.unsqueeze(dim=1).repeat(1, max_length, 1, 1)  ### YZ for listener
        if not already_dropfeat:
...
class SpeakerDecoder(nn.Module):
    ...
    def forward(self, words, ctx, ctx_mask, h0, c0):
        ...
        split_logit = self.split_projection(x)

        return logit, h1, c1, x, split_logit

        # x = self.drop(x)
        # logit = self.projection(x)

        # return logit, h1, c1, x