        ...
        for split in splits:
            for i_item, item in enumerate(load_datasets([split])):
                ...
                if "/" in split:
                    ...
                    import ast
                    for j, instr in enumerate(ast.literal_eval(item['new_instructions'])):  
                        try:
                            for sub_id, sub_instr in enumerate(instr):
                                mask_instr = ['[MASK]']*len(sub_instr)
                                instr[sub_id] = mask_instr
                                new_instr = list(map(lambda n:" ".join(n), instr))
                                new_full_instr = " ".join(new_instr)
                                target_instr = " ".join(sub_instr)
                                
                                ''' BERT tokenizer '''
                                instr_tokens = tokenizer.tokenize(new_full_instr)
                                target_tokens = tokenizer.tokenize(target_instr)

                                padded_instr_tokens, _ = pad_instr_tokens(instr_tokens, args.maxInput)
                                padded_target_tokens, _ = pad_instr_tokens(target_tokens, args.maxInput)

                                instr_encoding= tokenizer.convert_tokens_to_ids(padded_instr_tokens)
                                target_encoding = tokenizer.convert_tokens_to_ids(padded_target_tokens)

                                ''' Chunk view'''
                                sub_index = list(map(lambda n:n-1, item['chunk_view'][j][sub_id]))
                                sub_path = item['path'][sub_index[0]: sub_index[1]]

                                for view_id, view in enumerate(sub_path):
                                    new_item = dict(item)
                                    if view_id == len(sub_path)-1:
                                        break
                                    new_item['instr_id'] = '%s_%d_%d_%d' % (item['path_id'], j, sub_id, view_id)
                                    new_item['instructions'] = new_full_instr
                                    new_item['target'] = target_instr
                                    new_item['path'] = [view, sub_path[view_id+1]]
                                    new_item['instr_encoding'] = instr_encoding
                                    new_item['target_encoding'] = target_encoding
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if 'instr_encoding' in item:
                ...
                obs[-1]['target_encoding'] = item['target_encoding']
